...
/**
 * Provides the OpenAgenda keywords filter Block.
 *
 * @Block(
 *   id = "openagenda_keywords_filter_block",
 *   admin_label = @Translation("OpenAgenda - Keywords filter"),
 ...
 * )
 */
class OpenagendaKeywordsFilterBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    if ($node && $node->hasField('field_openagenda') && $this->routeMatch->getRouteName() == 'entity.node.canonical') {
      $block = [
        '#theme' => 'block__openagenda_keywords_filter',
      ];
    }
